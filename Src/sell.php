...
<?php 
	...
	$u_id = $_SESSION['user'];
			if(isset($_POST['sold']))
			{	
				$v_id = mysqli_real_escape_string($conn,$id);
				$nid =  mysqli_real_escape_string($conn,$_POST['nid']);
				$cf_name =  mysqli_real_escape_string($conn,$_POST['cf_name']);
				$cl_name =  mysqli_real_escape_string($conn,$_POST['cl_name']);
				$c_email =  mysqli_real_escape_string($conn,$_POST['c_email']);
				$c_mobile =  mysqli_real_escape_string($conn,$_POST['c_mobile']);
				$w_start =  mysqli_real_escape_string($conn,$_POST['w_start']);
				$w_end =  mysqli_real_escape_string($conn,$_POST['w_end']);
				...
				$invoice_id= "#IE9S".mysqli_real_escape_string($conn,$id). "S";
				$qrr="INSERT INTO customer(v_id,cf_name,cl_name,c_email,c_mobile,nid,w_start,w_end,payment_type,invoice_id,c_address,c_pass,extra) VALUES('$v_id','$cf_name','$cl_name','$c_email','$c_mobile','$nid','$w_start','$w_end','$payment_type','$invoice_id','$c_address','$c_pass','$extra')";
					echo $qrr;
					if(mysqli_query($conn,$qrr))
					{
				
					  $result = mysqli_query($conn,"UPDATE vehicle SET s_price = '" . $s_price . "',u_id = '" . $u_id . "',sold_date = '" . $w_start . "', status = 'Sold'  WHERE v_id =$v_id;");
						?>
