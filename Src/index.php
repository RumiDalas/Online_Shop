...
if(isset($_POST['btn-login']))
{
	...
	$email = trim($email);
	$upass = trim($upass);
	$sql="SELECT u_id, f_name, l_name, u_pass , u_type FROM users WHERE u_email='$email'";
	$res=mysqli_query($conn,$sql);
	$row=mysqli_fetch_array($res);
	
	$count = mysqli_num_rows($res); // if uname/pass correct it returns must be 1 row
	
	if($count == 1 && $row['u_pass']==$upass && $row['u_type']=="Admin")
	{
		$_SESSION['user'] = $row['u_id'];
		header("Location: dashboard.php");
	}
	else if($count == 1 && $row['u_pass']==$upass && $row['u_type']=="Employee")
	{
		$_SESSION['user'] = $row['u_id'];
		header("Location: dashboard.php");
	}
