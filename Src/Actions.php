...
try
{
	...
	$con = mysqli_connect("localhost","root","","vsms");   //change as per your localhost or server database settings
  //change as per your localhost or server database settings
	...
	else if($_GET["action"] == "create")
	{
		...
		$result = mysqli_query($con,"INSERT INTO manufacturer(manufacturer_name) VALUES('" . mysqli_real_escape_string($_POST["manufacturer_name"]) . "');");
		
		//Get last inserted record (to return to jTable)
		$result = mysqli_query($con,"SELECT * FROM manufacturer WHERE manufacturer_id = LAST_INSERT_ID();");
	...
	else if($_GET["action"] == "update")
	{
		...
		$result = mysqli_query($con,"UPDATE manufacturer SET manufacturer_name = '" . mysql_real_escape_string($_POST["manufacturer_name"]) . "' WHERE manufacturer_id = " . $_POST["manufacturer_id"] . ";");
	...
	{
		//Insert record into database
		$result = mysqli_query($con,"INSERT INTO model(model_name, manufacturer_name) VALUES('" . mysqli_real_escape_string($_POST["model_name"]) . "', '" . mysqli_real_escape_string($_POST["manufacturer_name"]) . "');");
		
		//Get last inserted record (to return to jTable)
		$result = mysqli_query($con,"SELECT * FROM model WHERE model_id = LAST_INSERT_ID();");
		$row = mysqli_fetch_array($result);
	...
	{
		//Update record in database
		$result = mysqli_query($con,"UPDATE model SET model_name = '" . mysqli_real_escape_string($_POST["model_name"]) . "', manufacturer_name = '" . mysqli_real_escape_string($_POST["manufacturer_name"]) . "' WHERE model_id = " . $_POST["model_id"] . ";");
	...
	else if($_GET["action"] == "checkuser")
	{
				if(!empty($_POST["username"])) {
				  ...
				  $result = mysqli_query($con,"SELECT count(*) FROM users WHERE u_email='" . $uname . "'");
	...
	$id = $_GET["id"];
	$result = mysqli_query($con,"SELECT * FROM vehicle WHERE v_id = $id;");
	$Row=mysqli_fetch_array($result);
	
	if(mysqli_query($con,"DELETE FROM vehicle WHERE v_id = $id;"))
		{
	...
	{
	    $id = $_GET["id"];
		$res=mysqli_query($con,"SELECT * FROM vehicle WHERE v_id = $id;");
        $vehicleRow=mysqli_fetch_array($res);
...
<body>
  <div class="modal-dialog modal-lg">
	...
	{ 
	    $id = $_GET["id"];
		$res=mysqli_query($con,"SELECT * FROM customer JOIN vehicle ON customer.v_id=vehicle.v_id JOIN users ON vehicle.u_id=users.u_id WHERE customer.v_id = $id;");
        $vehicleRow=mysqli_fetch_array($res);
